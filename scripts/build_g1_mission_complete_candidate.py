...
BASE_ROM = ROOT / "artifacts" / "generated" / "g1cred1.z64"
GE_STOCK = ROOT / "artifacts" / "roms" / "GoldenEye 007 (USA).z64"
GE_480I = ROOT / "artifacts" / "roms" / "BASELINE_GE_480i_direct_from_stock.z64"
OUT_ROM = ROOT / "artifacts" / "generated" / "g1mcfix1.z64"
REPORT = ROOT / "reports" / "tnd480i_g1mcfix1_mission_complete_20260519.json"

# GE480i menu0D mission-complete deltas that were just outside the previously
# ported briefing/objectives range. Without these, "Kill total" stays in the
# old 240p row band and overlaps the Best Time line.
MISSION_COMPLETE_OFFSETS = [
    (0x4C564, "Kill total label x: 0x37 -> 0x50"),
    (0x4C56C, "Kill total label y: y2 + 0xF4 -> y2 + 0x140"),
    (0x4C5EC, "Kill total value x: 0x82 -> 0x9B"),
    (0x4C5F4, "Kill total value y: y2 + 0xF4 -> y2 + 0x140"),
]
...
def make_bps(target, out_stem):
    ...
    subprocess.run(
        [
            ...
            str(manifest),
            "--metadata",
            f"TND6480i {out_stem}: g1cred1 plus GE480i mission-complete Kill total row coordinates.",
        ],
        check=True,
        cwd=ROOT,
    )
...
def apply_patch(rom, ge_stock, ge480):
    patches = []
    for offset, note in MISSION_COMPLETE_OFFSETS:
        old = word(rom, offset)
...
def main():
    ...
    patches = apply_patch(rom, args.ge_stock_rom.read_bytes(), args.ge480_rom.read_bytes())
    ...
    report = {
        ...
        "purpose": "Move the mission-complete Kill total label/value to the GE480i statistics row band.",
        "scope": "Only four menu0D mission-complete words at 0x4C564-0x4C5F4 are changed; gameplay, dossier, and credits code are untouched.",
        "patches": patches,
