...
def emit_aux_reg(reg):
    return [sw(reg, 0x0018, R_T0)]


def emit_aux_const(value):
    return [
        lui(R_T1, value >> 16),
...
def make_backdata_snapshot():
    ...
    words = [
        *unlock_sc64_regs(),
        *emit_aux_const(ascii_word("VDBA")),
        lui(R_T2, 0x8002),
        lw(R_T2, 0x32A8, R_T2),
        *emit_aux_reg(R_T2),
    ]
    for off in (0x0000, 0x0004, 0x0018, 0x001C, 0x0020, 0x0028):
        words.extend([
            lw(R_T3, off, R_T2),
            *emit_aux_reg(R_T3),
        ])
...
def make_hwvi_snapshot():
    ...
    words = [
        *unlock_sc64_regs(),
        *emit_aux_const(ascii_word("VHWV")),
        lui(R_T2, 0xA440),
    ]
    for off in (0x0000, 0x0004, 0x0008, 0x0024, 0x0028, 0x0030, 0x0034):
        words.extend([
            lw(R_T3, off, R_T2),
            *emit_aux_reg(R_T3),
        ])
...
def apply_aux_vidstate(rom, snapshot, hook):
    ...
    return {
        ...
        "debug_command": "sc64deployer.exe debug --no-writeback",
        "expected_aux_sequence": info["words"],
    }
...
def build(args):
    ...
    diagnostic = apply_aux_vidstate(rom, args.snapshot, args.hook)
...
def main():
    ...
    parser.add_argument("--hook", choices=("hvi", "bclr"), default="hvi")
